...
//     fn from(ThreadTaskRoot { data, _items: _ }: &'a ThreadTaskRoot<T>) ->
// Self {         let cap = ThreadTaskRoot::<T>::capacity();
//
//         // TODO remove bounds checking if present
//         let task_blocks_ptr =
// data[..cap].as_ptr().cast::<AtomicPtr<TaskBlock<T>>>();         let
// status_ptr = data[cap..].as_ptr().cast::<AtomicU32>();
//
//         Self {
...
//     //
//     //         // If somebody else already moved the head pointer along,
// that's fine     //         let _ = head.compare_exchange(
//     //             current,
//     //             (current + 1) % data_len,
//     //             Ordering::Relaxed,
//     //             Ordering::Relaxed,
...
//             (self.0 & (BLOCKS_MASK << (u32::BITS - 2 * num_blocks!()))) <<
// num_blocks!()         }
//
...
//             let zeros = (self.0 ^ (BLOCKS_MASK << (u32::BITS - 2 *
// num_blocks!()))).leading_zeros();             if zeros < num_blocks!() {
//                 Some(zeros)
...
//         #[test]
//         fn sanity() {
//             assert_eq!(StatusView(0xFF_FE_00_00).empty_blocks(),
// 0xFF_FE_00_00);             assert_eq!(
...
//
//             assert_eq!(StatusView((1 << 31) >> 4).next_empty_block(),
// Some(4));             assert_eq!(
//                 StatusView((1 << 31) >> (4 +
// num_blocks!())).next_full_block(),                 Some(4)
//             );
//
//             assert_eq!(StatusView((1 << 31) >> 4).next_full_block(), None);
//             assert_eq!(
//                 StatusView((1 << 31) >> (4 +
// num_blocks!())).next_empty_block(),                 None
//             );
//
//             assert_eq!(
//                 StatusView(0xFF_FE_00_00 >>
// num_blocks!()).best_available_block(),                 None
//             );
//             assert_eq!(StatusView(0x01_00_00_00).best_available_block(),
// Some(7));
